...
#define GRID_LIMITX 256
// #define GRID_LIMITY 16

__device__ float getA(float *a, int x, int y, int K0, int n, int k0) {
    if (x < 0 || y < 0 || x >= n || y >= n) return 0;
    else return a[(x*n + y) * K0 + k0];
}
...
__global__ void convolve(float* a, float* b, float* c, const int n, const int m, const int nz, const int K0, const int K, const int gridOffsetX//, const int gridOffsetY
    ...
    int ii = blockIdx.x, jj = blockIdx.y, k0 = threadIdx.x + gridOffsetX, k = k0, i, j;
    
    if (k0 >= K0 || k >= K) return;
    
    for (i = 0; i < m; i++)
        for (j = 0; j < m; j++)
            c[(ii * nz + jj) * K0 + k] += getA(a, ii-i, jj-j, K0, n, k0) * b[(i * m + j) * K + k];
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (c4d) {
        dimsc = new mwSize[3];
        dimsc[0] = nz; dimsc[1] = nz; dimsc[2] = K;
    ...
    cudaHostAlloc(&fc, sizeof(float)*size3d(szc, c4d, K), cudaHostAllocPortable);
    ...
    cudaMalloc(&dc, sizeof(float)*size3d(szc, c4d, K0));
    ...
    threadsPerBlock = min(K0, GRID_LIMITX);
    ...
    cudaMemcpy(fc, dc, sizeof(float)*size3d(szc, c4d, K), cudaMemcpyDeviceToHost);
    // t5 = clock();printf("5 ");
    
    if (fc) {
        for (i = 0; i < szc; i++)
            for (j = 0; j < K; j++)
                cc[i + j * szc] = (double)fc[i * K + j];
    }else
